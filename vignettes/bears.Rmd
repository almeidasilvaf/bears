...
download <- download_from_ena(metadata, ds$fastqdir)
```
...
As rRNA database, we will use an example file in this package's /extdata 
subdirectory.
...
Now that we have performed all quality checks, we're good to go.
