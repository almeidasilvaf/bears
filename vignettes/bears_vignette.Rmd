...
(e.g., `star_env`, `rseqc_env`, `salmon_env`).
...
To download the .fastq files with reads, you have 2 options:
...
runs *fastp* [@chen2018fastp] and saves filtered .fastq files 
...
First, let's use `trim_reads()` to filter reads.
...
```{r summary_stats_fastp, eval = TRUE}
